...
class PdfParser:
    ...
    def parse(self, file_path: str | Path) -> Document:
        ...
        try:
            ...
            logger.warning(f"Docling parsing failed: {e}. Falling back to unstructured.")
            if UNSTRUCTURED_AVAILABLE:
                logger.info(f"Parsing PDF with Unstructured: {file_path}")
                return self._parse_with_unstructured(file_path)
            else:
                logger.error("Unstructured not available. PDF parsing failed completely.")
                raise RuntimeError(f"Could not parse PDF. Both parsers failed or missing. Original error: {e}")
    ...
    def _parse_with_docling(self, file_path: Path) -> Document:
        ...
        try:
                ...
                for item, level in result.document.iterate_items():
                    ...
                    if item_type == "TextItem":
                        ...
                        if str(label) in ("title", "section_header"):
                            doc.blocks.append(Heading(content=item.text, level=1, score=100))
                        else:
            ...
            logger.info(f"Could not parse docling blocks precisely ({e}), falling back to raw markdown block.")
            md_text = result.document.export_to_markdown()
            doc.blocks.append(Paragraph(content=md_text, metadata={"format": "markdown"}))
                
        return doc

    def _parse_with_unstructured(self, file_path: Path) -> Document:
        languages = self.config.ocr_languages if hasattr(self.config, 'ocr_languages') else ["eng"]
        ...
        for element in elements:
            ...
            if element_type == "Title":
                doc.blocks.append(Heading(content=text, level=1, score=100, metadata=metadata))
